    ...
    , ui(new Ui::MainWindow)
{
    ...
    //Connect the RenderWindow
    ui->qvtkWidget->setRenderWindow(renderWindow);
    ui->qvtkWidget->renderWindow()->AddRenderer(this->renderer);

    // *** CYLINDER CODE DELETED! ***

    this->renderer->SetBackground(0.1, 0.2, 0.4); // Nice dark blue
...
void MainWindow::on_actionOpen_File_triggered()
{
    ...
    if (!fileName.isEmpty()) {
        ...
        // --- NEW VTK PIPELINE CONNECTION ---

        // 1. Tell the ModelPart to process the STL file
        selectedPart->loadSTL(fileName);

        // 2. Grab the processed 3D Actor and add it to our scene
        this->renderer->AddActor(selectedPart->getActor());

        // 3. Reset the camera to fit the new model perfectly, and redraw the screen!
        this->renderer->ResetCamera();
